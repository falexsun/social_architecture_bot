...
SYSTEM_PROMPT = """Ты — аналитик общественных изменений. Отвечай по-русски и используй приложенные материалы как внутреннюю методологию анализа.

Методология — это способ мышления, а не тема каждого ответа. Если пользователь не спрашивает прямо, что такое социальная архитектура, не объясняй и не определяй её, не добавляй блоки «Ключевая идея» и не пересказывай принципы учебника. Вместо этого решай конкретную проблему.
...
- используй материалы как внутреннюю базу знаний, но не упоминай книгу, презентацию, источники, авторов и номера страниц;
- не добавляй сноски, ссылки и формулировки «согласно книге», «по материалам» или «в источнике сказано».
...
- используй не более пяти смысловых блоков; допустимы короткие подписи вида «Практический смысл:»;
- пиши компактно, без повторения одной мысли разными словами;
- старайся уложить полный ответ в 5500 символов и обязательно закончи мысль;
- давай только один итоговый вывод и не повторяй его другими словами в конце.

Не раскрывай скрытые рассуждения. Давай краткое обоснование, вывод и, когда уместно, план действий."""

CASE_RESPONSE_INSTRUCTION = """Это запрос на решение кейса. Примени методологию молча и дай предметное решение.

Обязательные правила:
- не объясняй, что такое социальная архитектура, и не используй блоки «Ключевая идея» или «Практический смысл»;
- не повторяй универсальную схему из учебника как готовый ответ;
- отдели известные факты от предположений и не объявляй причину установленной без данных;
- назови 2–4 правдоподобные причинные гипотезы и укажи, как быстро их проверить;
- предложи конкретный механизм изменения поведения или системы: кто что делает, почему это должно сработать и какие условия необходимы;
- сочетай инфраструктуру, коммуникацию, стимулы и разумный контроль, если они уместны; не отвергай наказание или регулирование автоматически;
- предложи небольшой пилот с территорией, сроком, ответственным и измеримым исходным уровнем;
- каждая рекомендация должна относиться именно к данному кейсу, а не быть общей декларацией;
- используй максимум четыре блока: «Что нужно проверить», «Решение», «Пилот», «Метрики и риски».
"""
...
def build_messages(question: str, context_text: str, history: list[dict]) -> list[dict]:
    case_instruction = CASE_RESPONSE_INSTRUCTION if is_case_question(question) else ""
    return [
...
def is_case_question(question: str) -> bool:
...
def remove_case_meta_sections(text: str) -> str:
    """Drop textbook-like meta sections if a small model emits them anyway."""
    forbidden_headers = {
        "ключевая идея",
        "что такое социальная архитектура",
        "определение социальной архитектуры",
        "практический смысл",
    }
    result: list[str] = []
    skipping = False
    for line in text.splitlines():
        ...
        looks_like_header = line.strip().endswith(":") and len(normalized) <= 60
        if normalized in forbidden_headers:
            skipping = True
            continue
        if skipping and looks_like_header:
            skipping = False
        if not skipping:
            result.append(line)
    cleaned = re.sub(r"\n{3,}", "\n\n", "\n".join(result)).strip()
    ...
    paragraphs = [
        ...
        if not re.search(
            r"\bсоциальная архитектура\s*[—-]\s*(?:это|не)\b",
            paragraph,
            flags=re.IGNORECASE,
        )
    ]
        ...
        for attempt in range(MAX_CONTINUATIONS + 1):
            ...
            messages.extend([
                {"role": "assistant", "content": content},
                {
                    "role": "user",
                    "content": (
                        "Ответ оборвался по техническому лимиту. Напиши только непосредственное "
                        "продолжение незаконченной фразы и кратко заверши ответ в пределах трех "
                        "предложений. Не создавай заголовки и списки, не повторяй вывод или уже "
...
def telegram_plain_text(text: str) -> str:
    ...
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    text = re.sub(
        r"\s*[\[(](?:[^\])]{0,100}\b(?:источник|стр\.?|страниц[аеуы]?)[^\])]{0,100})[\])]",
        "",
        text,
        flags=re.IGNORECASE,
    )
...
async def answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_case_question(question):
            result = remove_case_meta_sections(result)
