...
4. Ответ возвращается в Telegram без упоминаний книги, источников и страниц.

Ответы отправляются обычным текстом без Markdown, HTML, таблиц и служебных символов разметки.
...
- `MAX_OUTPUT_TOKENS` — лимит основной генерации; по умолчанию 3200 токенов.
- `CONTINUATION_OUTPUT_TOKENS` — короткий лимит технического продолжения без повторов.
- `MAX_CONTINUATIONS` — число автоматических продолжений, если модель остановилась по лимиту.
- `MAX_HISTORY_MESSAGES` — число сообщений истории, передаваемых модели.
- `MAX_HISTORY_MESSAGE_CHARS` — ограничение одного сообщения истории.
